  ...
  export default {
    ...
    methods: {
      unitQualityofQuestionsReport() {
        let paper = this.options.find(i=>i.code == this.value);
        console.log({paperCode: this.value,artType:paper.artType},paper);
        this.$ajax.unitQualityofQuestionsReport({paperCode: this.value,artType:paper.artType})
