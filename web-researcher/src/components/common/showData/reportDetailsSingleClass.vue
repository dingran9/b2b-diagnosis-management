<template>
  <div class="rp-div">
    <section class="header" v-if="true">
      <div class="title">
        <span>试卷名称：{{paperDetail.diagnosisPaperName}}</span>
  ...
  export default {
    data() {
      return {
        ...
        userInfo:JSON.parse(this.localData.$user().get()),
        result:{},//初始化数据
    ...
    methods:{
      ...
      initData: function () {
        this.$ajax.getCommon2Get("teacherReport/getSingleClassReport")
          .then((res) => {
            console.log(res)
            this.result=res.result;
            this.notAttended=res.result.notAttendedStudentScoreModels.length;
            this.attended=res.result.studentScoreModels?res.result.studentScoreModels.length:0;
          }, (res) => {
            console.log(res);
          });
      }
