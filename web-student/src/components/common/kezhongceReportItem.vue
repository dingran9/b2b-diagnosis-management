<template>
  <div class="report-item" @click="beginAnswer">
    <div class="report-item-title ge-text-dian">
      {{(report.className)}}
      <span
        class="report-item-title-top"
        :class="{type2:type == 2}">
      {{report.status == 0 ? '未开始' : report.status == 1 ? '进行中' : '看报告'}}
      </span>
    </div>
    <div class="report-item-body">
      <div class="report-item-body-name ge-text-dian">
        {{report.inClassTestName}}
      </div>
      <div style="font-size: 14px;color:#999;padding:10px 0;">
        <span class="total-question">
          共 {{report.questionCount}} 题
        </span>
        <div class="time ge-pull-right">
          {{moment(report.updateTime).fromNow()}}
          {{moment(report.updateTime).format('a h:mm')}}
