    ...
    <div class="hg-div" v-if="isEmptyObject(result)!=true&&result!=null">
      ...
      <ul>
        ...
        <li v-for="(value,key,index) in result">
          ...
          <p class="hg-num">
            <span class="hg-num-l" v-bind:style="{width: value.standard+'%'}">{{value.standard}}%</span><span class="hg-num-r" v-bind:style="{width:100- value.standard+'%'}">{{100 - value.standard | jiequ}}%</span>
          </p>
        </li>
      </ul>
    </div>
  ...
  export default {
    ...
    methods: {
      getSourceDate: function () {
        this.$ajax.getCommonGet('teacherReport/getClassStandard')
          .then((res) => {
