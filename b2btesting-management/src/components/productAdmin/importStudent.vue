<template>
  ...
  <div>
    ...
    <p class="upLoadexplain">
      说明：请严格按照模板中的格式填写，填写完成载入文件后，点击确认按钮即可，批量导入的默认密码为学生工号。<a v-bind:href = getexlsx>下载模板</a>
    </p>
  ...
  export default {
    data() {
      return {
        ...
        getexlsx:this.rootUrl+"excelTemp/student_info.xlsx",
        fileType: null
      };
    },
    mounted(){
    ...
    methods: {
      ...
      upLoadFile(){
        ...
        this.$http.post(this.rootUrl + 'userManager/readExcelForStudent',
          form).then((res) => {
          if (res.body.code == 'Success' && res.body.httpCode == '200') {
            this.$message('上传成功');
            this.$router.push('/preAdminStuMange')
          }
        }, (res) => {
          console.log(res);
        });
