      ...
      <div class="chart-body">
        ...
        <div class="chart-main">
          ...
          <div
            ...
            class="chart-item">
            <h2 class="chartH2"
                :style="{width:((result.classModels.length*20+50)>74?(result.classModels.length*20+50):74)+'px'}">
              第{{index + 1}}次测试</h2>
            <div style="height: 50%;" class="scoreContent">
              <el-tooltip class="item" effect="light" :content="classes[index2].className+', '+score+'分'"
                          :placement="score>0?'top':'bottom'" v-for="(score,index2) in time">
                <div
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      let examType = JSON.parse(window.localStorage.getItem('reportData')).examType

      let path = 'teacherReport/getStandardScoreChange/' + teacherCode + "/" + subjectCode + "/" + releaseExamCode + '/' + examType
      ...
      this.$http.get(url)
        .then(res => {
          ...
          if (res.body.message == "Success") {
            ...
            if (res.body.result) {
              if (res.body.result.classModels) {
                for (var i = 0; i < res.body.result.classModels.length; i++) {
                  res.body.result.classModels[i].act = true;
                }
              }
              this.result = res.body.result;
              this.classAndTime = filterData(res.body.result);
